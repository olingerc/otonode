angular.module('oto').controller('SeriesCtrl', ['$scope', '$rootScope', '$http', '$timeout', function($scope, $rootScope, $http, $timeout) {
    ...
    $scope.searchSeries = function() {
        if ($scope.query) {
            ...
            $http.get('/api/watchlist/series/search', {
                    params: {
                        'query': $scope.query
                    }
                })
    ...
    $scope.addSeries = function(show) {
        var exists = _.find($scope.seriesCollection, function(inshow) {
            return inshow.seriesid == show.seriesid;
        });
        if (!exists) {
            $scope.savingAdd[show.seriesid] = 'Saving';
            $http.post('/api/watchlist/series/addseries', {
    ...
    $scope.removeSeries = function(show, showindex) {
        $http.post('/api/watchlist/series/remove', {
                showid: show.seriesid
            })
            .success(function(response) {
    ...
    $scope.setLastDownloaded = function(show) {
        ...
        $http.post('/api/watchlist/series/update', {
                showid: show.seriesid,
                lastdownloaded: show.activeEpisode
            })
            .success(function(response) {
    ...
    $scope.setLastWatched = function(show) {
        ...
        $http.post('/api/watchlist/series/update', {
                showid: show.seriesid,
                lastwatched: show.activeEpisode
            })
            .success(function(response) {
                $scope.savingUpdate[show.seriesid] = false;
            })
